...
# 2 "srcs/src/lin_alg.c" 2


void vec_add_scaled(c_float *c, const c_float *a, const c_float *b, c_int n, c_float sc) {
  c_int i; VITIS_LOOP_5_1: for (i = 0; i < n; i++) c[i] = a[i] + sc * b[i];
}
c_float vec_scaled_norm_inf(const c_float *S, const c_float *v, c_int l) {
  c_int i; c_float abs_Sv_i, max = 0.0;
  VITIS_LOOP_9_1: for (i = 0; i < l; i++) { abs_Sv_i = (((S[i] * v[i]) < 0) ? -(S[i] * v[i]) : (S[i] * v[i])); if (abs_Sv_i > max) max = abs_Sv_i; }
  return max;
}
c_float vec_norm_inf(const c_float *v, c_int l) {
  c_int i; c_float abs_v_i, max = 0.0;
  VITIS_LOOP_14_1: for (i = 0; i < l; i++) { abs_v_i = (((v[i]) < 0) ? -(v[i]) : (v[i])); if (abs_v_i > max) max = abs_v_i; }
  return max;
}
c_float vec_norm_inf_diff(const c_float *a, const c_float *b, c_int l) {
  c_float nmDiff = 0.0, tmp; c_int i;
  VITIS_LOOP_19_1: for (i = 0; i < l; i++) { tmp = (((a[i] - b[i]) < 0) ? -(a[i] - b[i]) : (a[i] - b[i])); if (tmp > nmDiff) nmDiff = tmp; }
  return nmDiff;
}
c_float vec_mean(const c_float *a, c_int n) {
  c_float mean = 0.0; c_int i;
  VITIS_LOOP_24_1: for (i = 0; i < n; i++) mean += a[i];
  return mean / (c_float)n;
}
void int_vec_set_scalar(c_int *a, c_int sc, c_int n) {
  c_int i; VITIS_LOOP_28_1: for (i = 0; i < n; i++) a[i] = sc;
}
void vec_set_scalar(c_float *a, c_float sc, c_int n) {
  c_int i; VITIS_LOOP_31_1: for (i = 0; i < n; i++) a[i] = sc;
}
void vec_add_scalar(c_float *a, c_float sc, c_int n) {
  c_int i; VITIS_LOOP_34_1: for (i = 0; i < n; i++) a[i] += sc;
}
void vec_mult_scalar(c_float *a, c_float sc, c_int n) {
  c_int i; VITIS_LOOP_37_1: for (i = 0; i < n; i++) a[i] *= sc;
}
void prea_int_vec_copy(const c_int *a, c_int *b, c_int n) {
  c_int i; VITIS_LOOP_40_1: for (i = 0; i < n; i++) b[i] = a[i];
}
void prea_vec_copy(const c_float *a, c_float *b, c_int n) {
  c_int i; VITIS_LOOP_43_1: for (i = 0; i < n; i++) b[i] = a[i];
}
void vec_ew_recipr(const c_float *a, c_float *b, c_int n) {
  c_int i; VITIS_LOOP_46_1: for (i = 0; i < n; i++) b[i] = (c_float)1.0 / a[i];
}
c_float vec_prod(const c_float *a, const c_float *b, c_int n) {
  c_float prod = 0.0; c_int i; VITIS_LOOP_49_1: for (i = 0; i < n; i++) prod += a[i] * b[i];
  return prod;
}
void vec_ew_prod(const c_float *a, const c_float *b, c_float *c, c_int n) {
  c_int i; VITIS_LOOP_53_1: for (i = 0; i < n; i++) c[i] = b[i] * a[i];
}
...
void mat_mult_scalar(c_float *Ax, const c_int *Ap, c_int An, c_float sc) {
  c_int i, nnzA = Ap[An];
  VITIS_LOOP_60_1: for (i = 0; i < nnzA; i++) Ax[i] *= sc;
}

void mat_premult_diag(c_float *Ax, const c_int *Ap, const c_int *Ai, c_int An, const c_float *d) {
  c_int j, i;
  VITIS_LOOP_65_1: for (j = 0; j < An; j++) {
    VITIS_LOOP_66_2: for (i = Ap[j]; i < Ap[j + 1]; i++) {
      Ax[i] *= d[Ai[i]];
    }
  }
}
...
void mat_postmult_diag(c_float *Ax, const c_int *Ap, c_int An, const c_float *d) {
  c_int j, i;
  VITIS_LOOP_74_1: for (j = 0; j < An; j++) {
    VITIS_LOOP_75_2: for (i = Ap[j]; i < Ap[j + 1]; i++) {
      Ax[i] *= d[j];
    }
  }
}
...
void mat_vec(const c_float *Ax, const c_int *Ap, const c_int *Ai, c_int An, c_int Am,
  ...
  c_int i, j;
  if (!plus_eq) {
    VITIS_LOOP_85_1: for (i = 0; i < Am; i++) y[i] = 0;
  }
  if (Ap[An] == 0) return;

  VITIS_LOOP_89_2: for (j = 0; j < An; j++) {
    VITIS_LOOP_90_3: for (i = Ap[j]; i < Ap[j + 1]; i++) {
...
void mat_tpose_vec(const c_float *Ax, const c_int *Ap, const c_int *Ai, c_int An, c_int Am,
  ...
  c_int i, j, k;
  if (!plus_eq) {
    VITIS_LOOP_101_1: for (i = 0; i < An; i++) y[i] = 0;
  }
  if (Ap[An] == 0) return;

  VITIS_LOOP_105_2: for (j = 0; j < An; j++) {
    VITIS_LOOP_106_3: for (k = Ap[j]; k < Ap[j + 1]; k++) {
      i = Ai[k];
...
c_float quad_form(const c_float *Px, const c_int *Pp, const c_int *Pi, c_int Pn, const c_float *x) {
  c_float quad_form = 0.;
  c_int i, j, ptr;
  VITIS_LOOP_118_1: for (j = 0; j < Pn; j++) {
    VITIS_LOOP_119_2: for (ptr = Pp[j]; ptr < Pp[j + 1]; ptr++) {
      i = Pi[ptr];
