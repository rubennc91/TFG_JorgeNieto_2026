...
# 2 "<built-in>" 2
# 1 "srcs/src/qdldl_interface.c" 2
...
# 2 "srcs/src/qdldl_interface.c" 2
...
void QDLDL_Ltsolve(const QDLDL_int n,
                   ...
                   const QDLDL_float* Lx,
                   QDLDL_float* x);
# 3 "srcs/src/qdldl_interface.c" 2
# 1 "srcs/lib\\qdldl_interface.h" 1
# 14 "srcs/lib\\qdldl_interface.h"
...
# 4 "srcs/src/qdldl_interface.c" 2
# 1 "srcs/lib\\util.h" 1
# 19 "srcs/lib\\util.h"
...
void c_strcpy(char dest[],
              const char source[]);
# 5 "srcs/src/qdldl_interface.c" 2
# 1 "srcs/lib\\kkt.h" 1
# 12 "srcs/lib\\kkt.h"
...
void update_KKT_param2(c_float *KKT_x,
                       ...
                       const c_int m);
# 6 "srcs/src/qdldl_interface.c" 2
...
c_int solve_linsys_qdldl(c_float * b) {
    ...
    VITIS_LOOP_37_1: for (j = 0 ; j < 15 ; j++) {
        b[j] = linsys_solver_sol[j];
    }

    VITIS_LOOP_41_2: for (j = 0 ; j < 19 ; j++) {
...
c_int update_linsys_solver_matrices_qdldl(void) {
    ...
    update_KKT_P(linsys_solver_KKT_x, Pdata_x, Pdata_p, 15,
                 linsys_solver_PtoKKT, linsys_solver.sigma, linsys_solver_Pdiag_idx, 10);

    update_KKT_A(linsys_solver_KKT_x, Adata_x, Adata_p, 15,
...
c_int update_linsys_solver_rho_vec_qdldl(const c_float * rho_vec){
    c_int i;
    VITIS_LOOP_65_1: for (i = 0; i < 19; i++){
