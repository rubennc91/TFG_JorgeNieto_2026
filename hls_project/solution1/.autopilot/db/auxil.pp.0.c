...
void update_z(void);
void update_y(void);
# 2 "srcs/src/auxil.c" 2
# 1 "srcs/lib\\auxil.h" 1
# 13 "srcs/lib\\auxil.h"
...
# 10 "srcs/lib\\proj.h" 2


void project(c_float *z);
# 4 "srcs/src/auxil.c" 2


# 1 "srcs/lib\\scaling.h" 1
# 12 "srcs/lib\\scaling.h"
c_int scale_data(void);
c_int unscale_data(void);
c_int unscale_solution(void);
# 7 "srcs/src/auxil.c" 2
# 1 "srcs/lib\\util.h" 1
# 19 "srcs/lib\\util.h"
...
static void compute_rhs(void) {
  c_int i;
  VITIS_LOOP_28_1: for (i = 0; i < data.n; i++) work_xz_tilde[i] = settings.sigma * work_x_prev[i] - qdata[i];
  VITIS_LOOP_29_2: for (i = 0; i < data.m; i++) work_xz_tilde[i + data.n] = work_z_prev[i] - work_rho_inv_vec[i] * work_y[i];
}
...
void update_x(void) {
  c_int i;
  VITIS_LOOP_39_1: for (i = 0; i < data.n; i++) {
...
void update_z(void) {
  c_int i;
  VITIS_LOOP_47_1: for (i = 0; i < data.m; i++) {
...
void update_y(void) {
  c_int i;
  VITIS_LOOP_55_1: for (i = 0; i < data.m; i++) {
...
c_float compute_pri_tol(c_float eps_abs, c_float eps_rel) {
  ...
  if (settings.scaling && !settings.scaled_termination) {
    ...
    max_rel_eps = (((max_rel_eps) > (temp_rel_eps)) ? (max_rel_eps) : (temp_rel_eps));
  } else {
...
c_int is_primal_infeasible(c_float eps_prim_inf) {
  ...
  VITIS_LOOP_126_1: for (i = 0; i < data.m; i++) {
    if (udata[i] > ((c_float)1e30) * (1e-04)) {
      if (ldata[i] < -((c_float)1e30) * (1e-04)) work_delta_y[i] = 0.0;
      else work_delta_y[i] = (((work_delta_y[i]) < (0.0)) ? (work_delta_y[i]) : (0.0));
    } else if (ldata[i] < -((c_float)1e30) * (1e-04)) work_delta_y[i] = (((work_delta_y[i]) > (0.0)) ? (work_delta_y[i]) : (0.0));
  }
  if (settings.scaling && !settings.scaled_termination) {
  ...
  if (norm_delta_y > ((c_float)1.0 / ((c_float)1e30))) {
    VITIS_LOOP_139_2: for (i = 0; i < data.m; i++) ineq_lhs += udata[i] * (((work_delta_y[i]) > (0)) ? (work_delta_y[i]) : (0)) + ldata[i] * (((work_delta_y[i]) < (0)) ? (work_delta_y[i]) : (0));
...
c_int is_dual_infeasible(c_float eps_dual_inf) {
  ...
  if (norm_delta_x > ((c_float)1.0 / ((c_float)1e30))) {
    if (vec_prod(qdata, work_delta_x, data.n) < cost_scaling * eps_dual_inf * norm_delta_x) {
      ...
      if (vec_norm_inf(work_Pdelta_x, data.n) < cost_scaling * eps_dual_inf * norm_delta_x) {
        ...
        VITIS_LOOP_169_1: for (i = 0; i < data.m; i++) {
          if (((udata[i] < ((c_float)1e30) * (1e-04)) && (work_Adelta_x[i] > eps_dual_inf * norm_delta_x)) ||
              ((ldata[i] > -((c_float)1e30) * (1e-04)) && (work_Adelta_x[i] < -eps_dual_inf * norm_delta_x))) return 0;
        }
        return 1;
      }
    }
...
void store_solution(void) {
  if ((info.status_val != (-3)) &&
    ...
    vec_set_scalar(xsolution, ((c_float)0x7fc00000UL), data.n);
    vec_set_scalar(ysolution, ((c_float)0x7fc00000UL), data.m);
    cold_start();
  }
}
...
void reset_info(OSQPInfo *info_ptr) {
  update_status(info_ptr, (-10));
}

void update_status(OSQPInfo *info_ptr, c_int status_val) {
  info_ptr->status_val = status_val;
  if (status_val == (1)) c_strcpy(info_ptr->status, "solved");
  else if (status_val == (2)) c_strcpy(info_ptr->status, "solved inaccurate");
  else if (status_val == (-3)) c_strcpy(info_ptr->status, "primal infeasible");
  else if (status_val == (3)) c_strcpy(info_ptr->status, "primal infeasible inaccurate");
  else if (status_val == (-10)) c_strcpy(info_ptr->status, "unsolved");
  else if (status_val == (-4)) c_strcpy(info_ptr->status, "dual infeasible");
  else if (status_val == (4)) c_strcpy(info_ptr->status, "dual infeasible inaccurate");
  else if (status_val == (-2)) c_strcpy(info_ptr->status, "maximum iterations reached");
  else if (status_val == (-5)) c_strcpy(info_ptr->status, "interrupted");
  else if (status_val == (-7)) c_strcpy(info_ptr->status, "problem non convex");
}
...
c_int check_termination(c_int approximate) {
  ...
  if ((info.pri_res > ((c_float)1e30)) || (info.dua_res > ((c_float)1e30))){
    update_status(&info, (-7)); info.obj_val = ((c_float)0x7fc00000UL); return 1;
  }
  ...
  if (prim_res_check && dual_res_check) {
    if (approximate) update_status(&info, (2));
    else update_status(&info, (1));
    exitflag = 1;
  } else if (prim_inf_check) {
    if (approximate) update_status(&info, (3));
    else update_status(&info, (-3));
    info.obj_val = ((c_float)1e30); exitflag = 1;
  } else if (dual_inf_check) {
    if (approximate) update_status(&info, (4));
    else update_status(&info, (-4));
    info.obj_val = -((c_float)1e30); exitflag = 1;
  }
  return exitflag;
}
