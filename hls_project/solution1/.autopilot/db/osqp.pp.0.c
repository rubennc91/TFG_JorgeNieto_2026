...
void osqp_set_default_settings(OSQPSettings *sets) {
  sets->rho = (c_float)(0.1);
  sets->sigma = (c_float)(1E-06);
  sets->scaling = (10);
  sets->max_iter = (4000);
  sets->eps_abs = (c_float)(1E-3);
  sets->eps_rel = (c_float)(1E-3);
  sets->eps_prim_inf = (c_float)(1E-4);
  sets->eps_dual_inf = (c_float)(1E-4);
  sets->alpha = (c_float)(1.6);
  sets->linsys_solver = (QDLDL_SOLVER);
  sets->scaled_termination = (0);
  sets->check_termination = (25);
  sets->warm_start = (1);
}
...
c_int osqp_solve(void) {
  ...
  VITIS_LOOP_33_1: for (iter = 1; iter <= settings.max_iter; iter++) {
  ...
  if (info.status_val == (-10)) {
      if (!check_termination(1)) update_status(&info, (-2));
  }
  store_solution();
  return exitflag;
}
...
c_int osqp_update_P(const c_float *Px_new, const c_int *Px_new_idx, c_int P_new_n) {
  c_int i, nnzP = Pdata_p[data.n];
  if (settings.scaling) unscale_data();
  if (Px_new_idx) {
    VITIS_LOOP_80_1: for (i = 0; i < P_new_n; i++) Pdata_x[Px_new_idx[i]] = Px_new[i];
  } else {
    VITIS_LOOP_82_2: for (i = 0; i < nnzP; i++) Pdata_x[i] = Px_new[i];
  }
...
c_int osqp_update_A(const c_float *Ax_new, const c_int *Ax_new_idx, c_int A_new_n) {
  ...
  if (settings.scaling) unscale_data();
  if (Ax_new_idx) {
    VITIS_LOOP_94_1: for (i = 0; i < A_new_n; i++) Adata_x[Ax_new_idx[i]] = Ax_new[i];
  } else {
    VITIS_LOOP_96_2: for (i = 0; i < nnzA; i++) Adata_x[i] = Ax_new[i];
  }
  if (settings.scaling) scale_data();
  update_linsys_solver_matrices_qdldl();
  reset_info(&info);
...
c_int osqp_update_rho(c_float rho_new) {
  ...
  settings.rho = ((((((rho_new) > ((1e-06))) ? (rho_new) : ((1e-06)))) < ((1e06))) ? ((((rho_new) > ((1e-06))) ? (rho_new) : ((1e-06)))) : ((1e06)));
  VITIS_LOOP_108_1: for (i = 0; i < data.m; i++) {
    if (work_constr_type[i] == 0) {
      work_rho_vec[i] = settings.rho;
      work_rho_inv_vec[i] = 1. / settings.rho;
    } else if (work_constr_type[i] == 1) {
      work_rho_vec[i] = (1e03) * settings.rho;
