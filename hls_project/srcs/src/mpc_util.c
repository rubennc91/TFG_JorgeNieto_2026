...
void inverse_matrix_2x2(c_float a, c_float b, c_float c, c_float d, c_float inv[2][2]) {
    ...
    c_float inv_determinant = 1.0f / determinant; // Usar 1.0f para float
...
// CORRECCIÓN: Eliminado argumento 'work'
void referencia(c_float* q_new, c_float ref){
    q_new[1] = -ref*Q2;
    q_new[4] = -ref*Q2;
    q_new[7] = -ref*Q2N;

    // CORRECCIÓN: Llamada sin 'work'
...
{
     // (Las variables locales se mantienen igual)
     c_float v_min[2];
     c_float v_max[2];
     c_float u_min[2];
     c_float u_max[2];
