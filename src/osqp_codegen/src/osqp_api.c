...
#else
  settings->linsys_solver  = 1;  /* En hardware forzamos el solver directo */
#endif
...
osqp_profiler_sec_push(OSQP_PROFILER_SEC_OPT_SOLVE);
  ...
  // If not warm start -> set x, z, y to zero
	if (!settings->warm_starting) {
		...
		warm_start_linsys_solver_qdldl((qdldl_solver*)work->linsys_solver, work->x);
	}
  // Main ADMM algorithm
...
OSQPInt osqp_update_data_mat(OSQPSolver*      solver,
  ...
  if(solver->settings->scaling){
      exitflag = update_linsys_solver_matrices_qdldl((qdldl_solver*)work->linsys_solver,
...
OSQPInt osqp_update_rho(OSQPSolver* solver,
  ...
  // Update rho_vec in KKT matrix
    exitflag = update_linsys_solver_rho_vec_qdldl((qdldl_solver*)work->linsys_solver, work->rho_vec, solver->settings->rho);
#ifdef OSQP_ENABLE_PROFILING
  if (work->rho_update_from_solve == 0)
    solver->info->update_time += osqp_toc(work->timer);
#endif /* ifdef OSQP_ENABLE_PROFILING */
